...
function normalizeAttribution(input) {
    ...
    for (const key of ['utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_term']) {
        const value = normalizeText(input[key], 160).replace(/[\u0000-\u001f\u007f]/g, '');
        if (value) result[key] = value;
    }
    const path = normalizeText(input.landing_path, 300).split(/[?#]/)[0].replace(/[\u0000-\u001f\u007f]/g, '');
    if (path.startsWith('/') && !path.startsWith('//')) result.landing_path = path;
    try {
        const url = new URL(normalizeText(input.referrer_origin, 500));
        if (['https:', 'http:'].includes(url.protocol)) result.referrer_origin = url.origin;
    } catch { /* A direct visit has no referrer. */ }
    if (['hero', 'audit', 'article-integration', 'calculator'].includes(input.cta_id)) result.cta_id = input.cta_id;
    if (['en', 'de', 'es'].includes(input.locale)) result.locale = input.locale;
    return result;
}

function renderAttribution(attribution) {
    const entries = Object.entries(normalizeAttribution(attribution));
    if (!entries.length) return '';
    return '<h3>Visit context</h3>' + entries.map(([key, value]) => `<p><strong>${key}:</strong> ${escapeHtml(value)}</p>`).join('');
}
...
function buildContactSubmission(body = {}) {
    ...
    return {
        ok: true,
        submission: { name: displayName, email, company, message, source, subject, attribution: normalizeAttribution(body.attribution) },
    };
}

module.exports = { TOOL_SOURCES, buildContactSubmission, escapeHtml, normalizeText, normalizeAttribution, renderAttribution };
